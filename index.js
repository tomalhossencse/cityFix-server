...
async function run() {
    try {
        ...
        const districtbyRegionCollection =
            cityFixDB.collection("districtbyRegion");
        ...
        const howItWorksStepsCollection =
            cityFixDB.collection("howItWorksSteps");
        ...
        app.get("/sttafs/:id", async (req, res) => {
            const { id } = req.params;
            const query = { _id: new ObjectId(id) };
            const result = await sttafsCollection.findOne(query);
            res.send(result);
        });
